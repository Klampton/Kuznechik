...
A high-performance C++ implementation of the GOST 34.12-2015 symmetric block cipher (widely known as Kuznechik). This repository provides a production-ready CLI utility for file encryption with support for advanced cipher modes and secure key derivation.
...
The project is designed with modularity and separation of concerns in mind:
    ...
    Kuznechik Engine: Implements the mathematical primitives (
    ...
    transformations), the non-linear S-box, and the Key Schedule using a Feistel network.
...
git clone https://github.com/your-username/kuznechik-gost.git
cd kuznechik-gost
...
# Encrypt an image using CBC mode
./kuznechik e photo.png -p "MySecurePass123" -m cbc

# Decrypt the resulting file
./kuznechik d photo_e_CBC.png -p "MySecurePass123" -m cbc
...
python tests/run_tests.py
    ...
    [!IMPORTANT]
    The test script verifies the result by calculating SHA-256 checksums of the original and the round-trip (decrypted) data. A PASS status ensures bit-perfect restoration.
